...
class VendaController {
  async index(request: Request, response: Response) {
    ...
    const vendasCliente = await VendaRepository.findAll(request.databaseName, {
  ...
  async mediaMes(request: Request, response: Response) {
    ...
    const months = eachMonthOfInterval({
      start: parsedStartDate || new Date(mediaMes[0].ano, mediaMes[0].mes - 1),
        ...
        parsedEndDate ||
        new Date(
          mediaMes[mediaMes.length - 1].ano,
          mediaMes[mediaMes.length - 1].mes - 1,
        ),
    });
    ...
    const precoMedioMes = months.map((monthDate) => {
      ...
      return {
        ...
        precoMedioKg: item?.precoMedioKg
          ? Number(item?.precoMedioKg.toFixed(3))
          : 0,
        precoMedioSaca: item?.precoMedioSaca
          ? Number(item?.precoMedioSaca.toFixed(3))
          : 0,
      };
    });

    const mediaSafra = mediaMes.reduce(
      (acc, curr) => ({
    ...
    const mediaSafraSaca =
      (mediaSafra.valorTotal / mediaSafra.quantidadeTotal) * 60;
