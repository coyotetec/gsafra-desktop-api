...
import {
  custoCategoriaQuery,
  custoPorTalhaoQuery,
} from '../../database/queries/custoProducaoQueries';
...
class CustoProducaoRepository {
  findCustoCategoria(
    databaseName: string,
    { idSafra, idTalhao, startDate, endDate }: FindCustoCategoriaArgs,
  ) {
    return new Promise<CustoCategoriaDomain[]>((resolve, reject) => {
      ...
      database.query(databaseName, query, [], (err, result) => {
        ...
        resolve(
          result.map((item) => CustoProducaoMapper.toCategoriaDomain(item)),
        );
      });
    });
  }
